...
print(crayons.cyan(f'[TriStar] [{time()}] TriStar made by razu. '
                   'FENDRA TEAM'))
print(crayons.cyan(f'[TriStar] [{time()}] Discord server: https://discord.gg/SnYKVzJ - For support, questions, etc.'))
...
async def event_ready() -> None:
    print(crayons.green(f'[TriStar] [{time()}] Client ready as {client.user.display_name}.'))
    ...
    for pending in list(client.pending_friends.values()):
        if pending.direction == 'INBOUND':
            ...
            if isinstance(friend, fortnitepy.Friend):
                print(f"[TriStar] [{time()}] Accepted friend request from: {friend.display_name}.")
            else:
                print(f"[TriStar] [{time()}] Declined friend request from: {pending.display_name}.")


@client.event
async def event_party_invite(invite: fortnitepy.PartyInvitation) -> None:
    await invite.accept()
    print(f'[TriStar] [{time()}] Accepted party invite from {invite.sender.display_name}.')


@client.event
async def event_friend_request(request: fortnitepy.PendingFriend) -> None:
    print(f"[TriStar] [{time()}] Received friend request from: {request.display_name}.")

    if data['friendaccept']:
        await request.accept()
        print(f"[TriStar] [{time()}] Accepted friend request from: {request.display_name}.")
    else:
        await request.decline()
        print(f"[TriStar] [{time()}] Declined friend request from: {request.display_name}.")
...
async def event_friend_message(message: fortnitepy.FriendMessage) -> None:
    ...
    print(f'[TriStar] [{time()}] {message.author.display_name}: {message.content}')
    ...
    if "!skin" in args[0].lower():
        try:
            ...
            print(f"[TriStar] [{time()}] Set skin to: {cosmetic.id}.")
            ...
            print(f"[TriStar] [{time()}] Couldn't find a skin with the name: {content}.")
        ...
        try:
            ...
            print(f"[TriStar] [{time()}] Set backpack to: {cosmetic.id}.")
            ...
            print(f"[TriStar] [{time()}] Couldn't find a backpack with the name: {content}.")
        ...
        try:
            ...
            print(f"[TriStar] [{time()}] Set emote to: {cosmetic.id}.")
            ...
            print(f"[TriStar] [{time()}] Couldn't find an emote with the name: {content}.")
        ...
        try:
            ...
            print(f"[TriStar] [{time()}] Set pickaxe to: {cosmetic.id}.")
            ...
            print(f"[TriStar] [{time()}] Couldn't find a pickaxe with the name: {content}.")
        ...
        try:
            ...
            print(f"[TriStar] [{time()}] Couldn't find an emoji with the name: {content}.")
        ...
        try:
            ...
            print(f"[TriStar] [{time()}] Set contrail to: {cosmetic.id}.")
            ...
            print(f"[TriStar] [{time()}] Couldn't find an contrail with the name: {content}.")
        ...
        print(f"[TriStar] [{time()}] Banner set to: {args[1]}, {args[2]}, {args[3]}.")
        ...
        print(f'[TriStar] [{time()}] Skin set to {args[0]}')
        ...
        print(f'[TriStar] [{time()}] Set variants of {args[1]} to {args[2]} {args[3]}.')
        ...
        await message.reply('For a list of commands, goto; https://discord.gg/SnYKVzJ')
        ...
        print(f'[TriStar] [{time()}] Status set to {content}.')
        ...
        print(f'[TriStar] [{time()}] Left the party as I was requested.')
        ...
        if member is None:
            ...
            try:
                ...
                print(f"[TriStar] [{time()}] Kicked user: {member.display_name}")
            except fortnitepy.Forbidden:
                await message.reply(f"Couldn't kick {member.display_name}, as I'm not party leader.")
                print(crayons.red(f"[TriStar] [{time()}] [ERROR] "
        ...
        if member is None:
            ...
            try:
                ...
                print(f"[TriStar] [{time()}] Promoted user: {member.display_name}")
            except fortnitepy.Forbidden:
        ...
        try:
            ...
            print(f'[TriStar] [{time()}] Party privacy set to {client.user.party.privacy}.')
...
if (data['email'] and data['password']) and (data['email'] != 'email@email.com' and data['password'] != 'password1'):
    try:
        ...
        print(crayons.red(f"[TriStar] [{time()}] [ERROR] {e}"))
else:
    print(crayons.red(f"[TriStar] [{time()}] [ERROR] Failed to login as no (or default) account details provided."))
